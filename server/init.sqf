...
//##############################################################################################
// SET THE GUN STORE OWNERSHIP AT START
//##############################################################################################
	diag_log format ["[GUNSTORE ] #### "];
	{
		diag_log format ["[GUNSTORE ] #### %1 ", vehicleVarName _x];
		...
		{
			diag_log "[GUNSTORE 1] SIDE = WEST";
			_x setVariable["Side", "WEST", true];
		};
		if (!isPlayer _x && (vehicleVarName _x =="GunStore2")) then
		{
			diag_log "[GUNSTORE 2] SIDE = WEST";
			_x setVariable["Side", "WEST", true];
		};
		if (!isPlayer _x && (vehicleVarName _x =="GunStore3")) then
		{
			diag_log "[GUNSTORE 3] SIDE = WEST";
			_x setVariable["Side", "WEST", true];
		};
		if (!isPlayer _x && (vehicleVarName _x =="GunStore4")) then
		{
			diag_log "[GUNSTORE 4] SIDE = WEST";
			_x setVariable["Side", "WEST", true];
		};
...
//##############################################################################################
// Monitor the gun store status for change and send notifications to players
//##############################################################################################
[] spawn
{
	[] execVM "server\functions\monitorGunStoreMarkers.sqf";
};
//##############################################################################################
